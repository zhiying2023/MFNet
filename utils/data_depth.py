...
def cv_random_flip(img, label, focal, depth):
    ...
    return img, label, focal, depth


def randomCrop(image, label, focal, depth):
    border = 30
    ...
    return image, label, focal_crop, depth_crop


def randomRotation(image, label, focal, depth):
    mode = Image.BICUBIC
    ...
    return image, label, focal, depth
...
class SalObjDataset(data.Dataset):
    ...
    def __init__(self, image_root, gt_root, focal_root, depth_root, trainsize):
        ...
        self.depths = sorted(self.depths) # 排序
    ...
    def __getitem__(self, index):
        ...
        image, gt, focal, depth = cv_random_flip(image, gt, focal, depth)
        image, gt, focal, depth = randomRotation(image, gt, focal, depth)
        image, gt, focal, depth = randomCrop(image, gt, focal, depth)
...
def get_loader(image_root, gt_root, focal_root, depth_root, batchsize, trainsize, shuffle=True, num_workers=0, pin_memory=True): #pin_memory设置为True可以将Tensor放入到内存的锁页区，加快训练速度

    dataset = SalObjDataset(image_root, gt_root, focal_root, depth_root, trainsize)
